...
```
RAG-based PDF Question Answering System | github.com/YOUR_USERNAME/rag-pdf-qa
LangChain · FAISS · SentenceTransformers · HuggingFace · Gradio

• Built an end-to-end RAG pipeline for querying PDF documents using
  LangChain, FAISS, and SentenceTransformers, achieving 87% Recall@3.
• Implemented semantic search with all-MiniLM-L6-v2 embeddings and FAISS
  vector indexing, reducing retrieval latency ~60% vs. brute-force search.
• Evaluated retrieval quality using Recall@K and MRR, iterating on chunk
  size and overlap to optimize relevance scores.
• Deployed an interactive Q&A interface with Gradio with shareable URL.
```
